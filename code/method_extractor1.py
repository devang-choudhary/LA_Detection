...
def extract_java_methods(java_file_path, csv_output_path, include_csv_fields=True):
    # Update the method_regex to more accurately capture complex return types, including generics and arrays
    method_regex = r'(public|protected|private|static|\s)?\s*([\w\[\]\<\>\,\s]+)\s+(\w+)\s*\(([^)]*)\)\s*\{?'
    class_regex = r'class\s+\w+'
    return_regex = r'return [^;]+;'
    # Enhance the comment regex to accurately ignore all comment types, including inline and block comments
    comment_regex = r'//.*|/\*[\s\S]*?\*/'
    ...
    for line in lines:
        ...
        # Extract and ignore comments to prevent them from interfering with method detection
        ...
        if current_method is None:
            ...
            if method_match:
                ...
                current_method = {
                    ...
                    'returns': '',
                    'comments': '',  # Comments are now ignored for method detection
                    'content': '',
                    'path': java_file_path
                }
                brace_count = line.count('{') - line.count('}')
                method_content = [line.strip()] if brace_count > 0 else []
                continue

        if current_method is not None:
            if '{' in line or '}' in line or method_content:
...
#     print_info, csv_file_path = extract_java_methods(java_file_path, "output_methods.csv", include_csv_fields=True)
#     files_info.append({"file_name": java_file_path, "info": print_info})
# print(files_info)
